...
async function registerUser(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  if (password.length < 6) {
    return res.status(400).json({ message: 'La contrasena debe ser de al menos 6 caracteres'})
  }
  ...
  if (!validations.isValidEmail( email )) {
    return res.status(400).json({ message: 'El correo no tiene el formato adecuado'})
  }
